...
void OLED_Write(int16_t x, int16_t y, char value) {
    ...
    if (cfont.y_size % 8 == 0) {
        ...
        for (rowcnt = 0; rowcnt < (cfont.y_size / 8); rowcnt++) {
            for (cnt = 2; cnt < cfont.x_size - 2; cnt++) //bounds were originally 0,cfont.xsize
            {
                temp = *(cfont.font + font_idx + cnt + (rowcnt * cfont.x_size));
                for (b = 0; b < 8; b++) {
                    if (temp & (1 << b)) {
                        if (cfont.inverted == FALSE){
                            OLED_DrawPixel(x + cnt - 2, y + (rowcnt * 8) + b, TRUE); //x position was originally x+cnt
                        }
                        else{
                            OLED_DrawPixel(x + cnt - 2, y + (rowcnt * 8) + b, FALSE); //x position was originally x+cnt
                        }
                    } else {
                        if (cfont.inverted == FALSE){
                            OLED_DrawPixel(x + cnt - 2, y + (rowcnt * 8) + b, FALSE); //x position was originally x+cnt
...
void OLED_Write_Text(int16_t x, int16_t y, char *text) {
    ...
    for (cnt = 0; cnt < length; cnt++){
        OLED_Write(x + (cnt * (cfont.x_size))- (4 * cnt), y, *text++); // x position was originally x + (cnt*(cfont.x_size))
    }
}
